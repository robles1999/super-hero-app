...
const SEARCH_BASE_URL = `https://www.superheroapi.com/api.php/${SUPERHERO_TOKEN}/search/`;
...
//---------------- capitalize firs letter of word  ---------
const capitalizeFirstLetter = (stat) => {
  let firstChar = stat.slice(0, 1).toUpperCase();
  let nextChars = stat.slice(1);
  const powerStatFirstCap = " - " + firstChar + nextChars;
  return powerStatFirstCap;
};
//---------------- show superhero  info  -------------------
const showSuperheroInfo = (superhero) => {
  ...
  const superheroStats = Object.keys(superhero.powerstats).map((stat) => {
    // capitalize first letter of the powerstat
    const capitalize = capitalizeFirstLetter(stat);
    return `<p>${superheroStatsEmoji(stat)}${capitalize}: 
        ${superhero.powerstats[stat]}</p>`;
  });

  const imgContainer = document.getElementById("img-container");
  // inserts the `h1`, `img` and `p` tags into the DOM
  imgContainer.innerHTML = `${superheroName} <img src="${
    superhero.image.url
